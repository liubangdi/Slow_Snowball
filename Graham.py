...
def initialize(account):
    # get_iwencai('A股,股票简称不包含st,上市天数>200,交易状态包含交易')  # iwencai回测区间最长五年
    ...
    run_monthly(func=handle, date_rule=1,
                reference_security='000001.SZ')  # 每月第一个交易日运行
    # g.buyframe=pd.DataFrame()


# 设置买卖条件，每个交易频率（日/分钟/tick）调用一次，目前没在用
def check(account, data):
    ...
    for stock in list(set(account.positions)):
        ...
        if rev > 0.10:
            ...
            # 股票仓位增加需要买入的比例
            order_target_value(
                stock, account.positions[stock].position_value*2)
            log.info('止损')
...
def MarketSignal(account):
    ...
    pct=history(stock_list, ['quote_rate'], 1, '1d',
