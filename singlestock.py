...
def initialize(account):
    account.security = '601766.SH'  # 可更换其他标的
...
def p_data(stock):
    ...
    Tdays = get_trade_days('20090501', '20180604')
    for day in Tdays:
        q = query(valuation.date, valuation.pb, valuation.ps,
                  growth_one_season.opt_profit_growth_ratio).filter(valuation.symbol == stock)
    ...
    context.bk_len = 1000
    ####################################################
    pbegin = context.pbegin*100
    pend = context.pend*100
    data['PS_R'] = data.valuation_ps / \
        (data.growth_one_season_opt_profit_growth_ratio/100+1.)
    data['PB_R'] = data.valuation_pb / \
        (data.growth_one_season_opt_profit_growth_ratio/100+1.)
    ...
    for i in range(bk_len):
        ...
        PB_x.sort()
        if len(PS_x) > 0:
            PB_xmin[i+1] = np.percentile(PB_x, pbegin)
            PB_xmax[i+1] = np.percentile(PB_x, pend)
            PS_xmin[i+1] = np.percentile(PS_x, pbegin)
            PS_xmax[i+1] = np.percentile(PS_x, pend)
        else:
            PS_xmin[i+1] = np.nan
            PS_xmax[i+1] = np.nan
            PB_xmin[i+1] = np.nan
    ...
    for i in range(int(len(data.index))-bk_len-1):
        PS_x = PS_x0[i:i+bk_len+1]
        PS_x = PS_x.copy()
        PS_x = np.array(pd.Series(PS_x).dropna())
        PS_x.sort()
        PB_x = PB_x0[i:i+bk_len+1]
        PB_x = PB_x.copy()
        PB_x = np.array(pd.Series(PB_x).dropna())
        PB_x.sort()
        if len(PS_x) > 0:
            PB_xmin[i+bk_len+1] = np.percentile(PB_x, pbegin)
            PB_xmax[i+bk_len+1] = np.percentile(PB_x, pend)
            PS_xmin[i+bk_len+1] = np.percentile(PS_x, pbegin)
            PS_xmax[i+bk_len+1] = np.percentile(PS_x, pend)
        else:
...
def p_update(stock):
    data_pv = pd.read_csv('./个股配置策略数据/'+str(stock)+'.csv')
    today = get_datetime().strftime('%Y%m%d')
    last_date = (datetime.datetime.strptime(
        (data_pv.index)[-1], '%Y-%m-%d') + datetime.timedelta(days=1)).strftime('%Y%m%d')
    ...
    for i in range(bk_len):
        ...
        PB_x.sort()
        if len(PS_x) > 0:
            log.info(np.percentile(PB_x, pend))
            PB_xmin[i+1] = np.percentile(PB_x, pbegin)
            PB_xmax[i+1] = np.percentile(PB_x, pend)
            PS_xmin[i+1] = np.percentile(PS_x, pbegin)
            PS_xmax[i+1] = np.percentile(PS_x, pend)
...
def handle_data(account, data):
    ...
    PB_p = 1.-(data.PB_R[previous_date]-data.PB_Rmin[previous_date]) / \
        (data.PB_Rmax[previous_date]-data.PB_Rmin[previous_date])
