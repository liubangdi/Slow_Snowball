...
def handle(account, data):
    ...
    if account.b == True:  # 买入信号为真
        ...
        if g.count % 12 == 0 or g.f_signal == True:
            ...
            data = get_extras(fl, None, yesterday, [
                              'acc_net_value', 'pre_net_value'], count=1)
            df = pd.DataFrame()
            for key, value in data.items():
                try:
                    vol = (get_candle_stick(key, end_date=yesterday, fre_step='1d', fields=[
        ...
        # 根据目标持仓权重，逐一委托下单
        frame_position = pd.DataFrame({'stock': buylist, 'position': list(
            [float("nan")]*(len(buylist)))}).set_index('stock')
...
def GrahamStockFilter(account, overflow=0):
    ...
    price = get_price(stk, None, date, '1d', [
                      'is_paused', 'is_st'], False, None, 1, is_panel=1)
    ...
    yesterday = get_last_datetime().strftime('%Y%m%d')
    q = query(valuation.symbol, balance.total_equity, income.basic_eps, valuation.pb, valuation.market_cap, income_one_season.profit_from_operations,
              valuation.capitalization, profit_one_season.roe, growth_one_season.opt_profit_growth_ratio).filter(valuation.symbol.in_(stock_list)).order_by(valuation.symbol)
...
def get_signal(account, data):
    ...
    close = history(stock_list, ['close'], 20, '1d',
                    False, 'pre', is_panel=1)['close']
    # 计算涨跌幅
    ...
    yesterday = get_last_datetime().strftime('%Y%m%d')
    q = query(valuation.symbol, balance.total_equity, valuation.market_cap).filter(
        valuation.symbol.in_(hlist)).order_by(valuation.symbol)
    df1 = get_fundamentals(q, date=yesterday)
    avg = sum(df1['valuation_market_cap'])/len(df1['valuation_market_cap'])

    q2 = query(valuation.symbol, balance.total_equity, valuation.market_cap).filter(
